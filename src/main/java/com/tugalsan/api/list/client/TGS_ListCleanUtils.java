...
public class TGS_ListCleanUtils {
    ...
    public static List deleteIf(TGS_CallableType1_Validate<Integer> deleteIf, List main, TGS_CallableType1_Run<Integer> optionalExecutionForEveryDeleteOp) {
    ...
    public static <T> List<T> cleanNulls(List<T> list, TGS_CallableType1_Run<Integer> optionalExecutionForEveryDeleteOp) {
        TGS_StreamUtils.reverse(0, list.size()).forEach(i -> {
            var o = list.get(i);
            if (o == null) {
                list.remove(i);
